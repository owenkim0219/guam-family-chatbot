...
⚠️ langgraph.prebuilt.create_react_agent는 v1.0+ 이후 deprecation 대상이지만
   여전히 작동. "create_agent (LangGraph prebuilt)" 표기는 이
   함수를 가리킴 (v1.0의 langchain.agents.create_agent는 v1.1.0에서 다시 제거됨).

SYSTEM_PROMPT 강화 이력:
- 1차: 규칙 1번에 "수치+고유명사" 구체화, 규칙 2번 도구 호출 강제, 양식 압축
  → 시연 #4 도구 호출 누락 해결, 가격 환각은 잔존
- 2차: 규칙 4번 신설 — "예상·평균·추정·대략 등 우회 표현 금지" + max_tokens 1024→512
  로 환각 면적 축소
"""
